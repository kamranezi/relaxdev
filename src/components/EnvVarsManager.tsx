    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-black/30 p-2 rounded-lg">
        <div className="flex gap-2">
          ...
          <Button variant={mode === 'raw' ? 'secondary' : 'ghost'} size="sm" onClick={() => setMode('raw')} className="text-sm">
            <FileText className="h-4 w-4 mr-2" />{t.envRawMode}
          </Button>
        </div>
        {mode === 'raw' && (
           <div className="flex gap-2">
             <input type="file" ref={fileInputRef} className="hidden" accept=".env,text/plain" onChange={handleFileUpload} />
             <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} className="bg-transparent border-gray-600 hover:bg-gray-800">
               <Upload className="h-4 w-4 mr-2" />{t.uploadEnv}
             </Button>
             <Button size="sm" onClick={handleSaveRaw} disabled={isSaving} className="bg-white text-black hover:bg-gray-200">
               {isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4 mr-2" />}{t.saveChanges}
             </Button>
           </div>
        )}
      </div>
